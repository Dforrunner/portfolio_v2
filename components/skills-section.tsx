      ...
      <div className="container mx-auto px-4">
        <div className={`transition-all duration-1000 ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}>
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold mb-4 animate-scale-in">Skills & Expertise</h2>
            ...
            {skillCategories.map((category, categoryIndex) => (
              <Card
                ...
                className={`hover:shadow-lg transition-all duration-300 hover-lift hover-glow ${
                  isVisible ? `animate-fade-in-up animate-stagger-${categoryIndex + 3}` : "opacity-0"
                }`}
              >
                <CardHeader className="text-center pb-4">
                  <category.icon
                    className={`h-10 w-10 sm:h-12 sm:w-12 mx-auto mb-3 sm:mb-4 ${category.color} animate-bounce-in animate-stagger-${categoryIndex + 1}`}
                  />
                  <CardTitle className="text-xl sm:text-2xl animate-fade-in-up animate-stagger-2">
                    {category.title}
                  </CardTitle>
                </CardHeader>
